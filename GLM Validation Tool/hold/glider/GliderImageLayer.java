...
public class GliderImageLayer extends AbstractLayer
{
    ...
    protected GliderSurfaceImageLayer imageLayer = new GliderSurfaceImageLayer();
    ...
    public void setImage(GliderImage image) throws IOException
    {
        if (image.getImageSource() instanceof String)
            ...
            ((SurfaceImageLayer) this.getImageLayer()).addImage(image.getName(), (BufferedImage) image.getImageSource(),
                image.getSector());
        else if (image.getImageSource() instanceof ArrayList)
            ((GliderSurfaceImageLayer) this.getImageLayer()).addImage(image.getName(), (ArrayList<AlignedImage>) image.getImageSource());
        else
